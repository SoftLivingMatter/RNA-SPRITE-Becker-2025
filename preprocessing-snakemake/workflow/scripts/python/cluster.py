...
class Position:
    ...
    def __init__(self, read_type, strand, chromosome, start_coordinate, end_coordinate, equality="position"):
        self._type = read_type
        self._strand = strand
        self._chromosome = chromosome
        self._start_coordinate = start_coordinate
        self._end_coordinate = end_coordinate
        self._consider_position = equality == "position"

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False

        if self._consider_position:
        ...
        return (self._type == other._type and
                self._strand == other._strand and
                self._chromosome == other._chromosome)


    def __hash__(self):
        if self._consider_position:
            return hash((self._type, self._strand, self._chromosome, 
    ...
    def to_string(self):
        try:
            feat = self._strand
            out = '{type}[{feat}]_{chrom}:{start}-{end}'.format(
                type=self._type,
                feat=feat,
                chrom=self._chromosome,
                start=self._start_coordinate,
                end=self._end_coordinate,
            )
        except:
            print(self._type, self._chromosome)
...
def get_clusters(bamfile, num_tags, equality):
        ...
        try:
            with pysam.AlignmentFile(bam, "rb") as f:
                for read in f.fetch(until_eof = True):
                    ...
                    read_type = 'DPM' if 'DPM' in barcode else 'RPM'
                    strand = '+' if not read.is_reverse else '-'
                    position = Position(read_type, strand, read.reference_name,
...
def write_clusters_to_file(clusters, outfile, unique=False):
    ...
    count = 0
    open_file = lambda filename: open(filename, 'w')
    if outfile.endswith('.gz'):
        open_file = lambda filename: gzip.open(filename, 'wt')
    
    with open_file(outfile) as f:
